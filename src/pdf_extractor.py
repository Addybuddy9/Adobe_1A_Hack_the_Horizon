...
class PDFOutlineExtractor:
    ...
    def __init__(self, config: Optional[ExtractorConfig] = None):
        ...
        self.cache = OutlineCache(self.config.cache_dir) if self.config.cache_enabled else None
    ...
    def extract_outline(self, pdf_path: str) -> Dict[str, Any]:
        ...
        # Try cache first
        if self.cache:
            cached_result = self.cache.get_cached_outline(pdf_path)
            if cached_result:
        ...
        try:
            with fitz.open(pdf_path) as doc:
                ...
                # Cache result
                if self.cache:
                    self.cache.cache_outline(pdf_path, result)
